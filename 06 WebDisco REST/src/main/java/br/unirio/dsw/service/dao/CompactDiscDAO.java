...
public class CompactDiscDAO extends AbstractDAO
{
	...
	/**
	 * Conta o número de CDs armazenados no sistema
	 */
	public int conta(String filtroNome)
	{
		...
		try
		{
			...
			ResultSet rs = ps.executeQuery();
			int contador = rs.next() ? rs.getInt(1) : 0;
			c.close();
			return contador;
		} 
		catch (SQLException e)
		{
			log("CompactDiscDAO.conta: " + e.getMessage());
			return 0;
		}
	}
	 ...
	 */
	public List<CompactDisc> lista(int pagina, int tamanho, String filtroNome)
	{
		String SQL = "SELECT * " +
					 "FROM CompactDisc " +
					 "WHERE title like ? " +
